// 在node 中使用的是common.js规范 引入的时候必须加‘./’
  ...
  // 请求 发送浏览器 write后必须调用response.end()结束
  // 请求不同的路径处理不同路由功能
  // 4.绑定端口号，启动服务器
  // 代码被修改，重启服务器
  // 1.加载http 模块
  ...
  // 3.服务器功能 request.url 获取的是端口号之后的内容，相应内容只能是字符串 'Content-Type','text/html;charset=utf-8'
  server.on('request', function (request,response) {
    var url=request.url;
    if(url=='/bigThree'){
      fs.readFile('./ressources/1.jpg',function(err,data){
        if(err){
      ...
      response.write('<b>hjhh</b>');
      response.end(JSON.stringify(['a', 'b', '实验台']));
    }
    
  })
  // 4.绑定端口号，启动服务器
server.listen(803,function (data) { 
  console.log('启动服务器成功');
  
 })
//  处理文件的时候 fs.read
// oschina->os在线工具->Content-Type 图片不需要指定编码格式，编码指的是：字符编码